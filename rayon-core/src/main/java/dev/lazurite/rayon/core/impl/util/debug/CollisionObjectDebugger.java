...
public final class CollisionObjectDebugger {
    ...
    public void renderSpace(MinecraftSpace space, float tickDelta) {
        ...
        var bodies = new ArrayList<PhysicsCollisionObject>();
        bodies.addAll(space.getRigidBodiesByClass(ElementRigidBody.class));
        bodies.addAll(space.getTerrainObjects().stream().map(TerrainObject::getCollisionObject).toList());
